...
# GUI setup
root = tk.Tk()
root.title("🖐 Sign Language Recognition with Voice")
...
SPEAK_INTERVAL = 1.5  # seconds between speaking

def speak(text):
    global last_spoken_time
    now = time.time()
    if now - last_spoken_time > SPEAK_INTERVAL:
        engine.say(text.replace("_", " "))
        engine.runAndWait()
...
def predict_sign(roi):
    ...
    if confidence >= CONFIDENCE_THRESHOLD:
        ...
        if prediction != prev_prediction:
            ...
            threading.Thread(target=speak, args=(prediction,), daemon=True).start()
